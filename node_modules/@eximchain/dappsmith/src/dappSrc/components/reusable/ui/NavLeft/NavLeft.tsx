...
function item(icon: IconOptions, text: string){return(
...
export default class NavLeft extends React.Component<Props, State>  {
    ...
    getFilteredFxns():NavLeftMenuItem[]{
      let items:NavLeftMenuItem[];
        if (this.state.searchVal === ""){
          items = sortBy(this.props.menuItems, (item:NavLeftMenuItem)=>item.title.toLowerCase());
        } else {
          ...
          const similiarityRatings = keyBy(stringCompare.findBestMatch(this.state.searchVal, itemNames).ratings, 'target');

          // Only show at least Golden Ratio similarity, sort by similarity
          items = sortBy(this.props.menuItems.filter((item:NavLeftMenuItem) => {
            return this.state.searchVal.length < 4 || similiarityRatings[item.title].rating > 0.1;
          }), (item:NavLeftMenuItem) => { return 1 - similiarityRatings[item.title].rating });
        }
      return items;
    }
    ...
    render(){
        ...
        let searchBar = (
          <Input type='text' value={this.state.searchVal} onChange={this.updateSearch} placeholder='Search' />
        );
        ...
        return(
            ...
            <div className="left-sidebar-body">
              ...
              <Nav selectedKey={selectedKey} onSelect={(event: any, key: any) => console.log(event, key)}>
                { navItems }
              </Nav>
            </div>
